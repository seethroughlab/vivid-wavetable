// SubOsc MIDI-input + voice-breakout tests.
//
// Verifies that:
//   1. SubOsc declares notes_in + voices_out + the four voice_* breakouts
//      (all voice_* / voices_out tagged ADVANCED).
//   2. A note-on via notes_in produces stereo audio on `output`.
//   3. A 3-note chord aligns voice_ids ascending; voice_freqs match held
//      notes; voices_out channels 0..2 carry non-silent audio; channels
//      3..15 are silent.
//   4. Note-off triggers ADSR release: stereo output decays.
//   5. Legacy lane-array path still emits per-voice audio when notes_in is absent.
...
// Minimal harness that wires SubOsc's two output audio ports + four lane
// outputs. Mirrors vivid-core's test_synth_breakouts FmHarness shape.
struct SubHarness {
    ...
    // SubOsc port indices: notes_in=0, frequencies=1, gates=2, velocities=3,
    // lane_ids=4, pitch_mod_audio=5. Input audio buffers[5] needs to be
...
int main(int argc, char** argv) {
    ...
    const std::string dylib_path = staging + "/sub_osc.dylib";
    ...
    check(desc != nullptr, "SubOsc descriptor not null");
    if (!desc) return 1;
    check(std::strcmp(desc->name, "SubOsc") == 0, "operator name is SubOsc");

    // ----- Surface check -----
    {
        std::fprintf(stderr, "\n--- SubOsc: declares breakout ports ---\n");
    ...
    {
        std::fprintf(stderr, "\n--- SubOsc: 3-note chord aligns voice_*/voices_out ---\n");
        SubHarness h;
        auto params = make_params(desc, {
            {"waveform", 0.0f},   // sine
            {"level",    0.5f},
            {"octave",   0.0f},   // -1 octave (sub_div = 2)
        ...
        h.clear_notes();
        h.zero_outputs();
        loader.process_audio(inst, &h.ctx);  // sustain block
        ...
        if (h.voice_ids_buf.data.size() == 3) {
            ...
            // ids 10, 20, 30 → ascending → freqs at C4, E4, G4 (sub at -1 oct)
        ...
        // voices_out channels 0..2 audible, 3..15 silent.
        for (int ch = 0; ch < 3; ++ch) {
            float r = h.voices_channel_rms(ch);
            check(r > 0.01f, (std::string("voices_out ch ") + std::to_string(ch)
                              + " audible").c_str());
        }
        ...
        check(h.stereo_rms() > 0.01f, "stereo output is audible during chord");
    ...
    {
        std::fprintf(stderr, "\n--- SubOsc: note-off triggers release ---\n");
        SubHarness h;
        auto params = make_params(desc, {
            {"waveform", 0.0f},
            {"level", 0.5f},
    ...
    // ----- Legacy lane-array path still works -----
    {
        std::fprintf(stderr, "\n--- SubOsc: legacy lane-array path ---\n");
        SubHarness h;
        h.disable_midi();
        // Set up one voice on the lane inputs (port indices: freq=1, gate=2,
        // vel=3, lane_id=4).
        ...
        h.zero_outputs();
        loader.process_audio(inst, &h.ctx);
        // Lane-driven output goes into voices_out channel 0; stereo also sums it.
        float ch0_rms = h.voices_channel_rms(0);
        check(ch0_rms > 0.01f, "lane path emits audio on voices_out ch0");
        check(h.stereo_rms() > 0.01f, "lane path also sums into stereo output");
        loader.destroy_instance(inst);
    }
