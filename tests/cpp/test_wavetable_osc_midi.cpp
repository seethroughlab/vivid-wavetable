...
int main(int argc, char** argv) {
    ...
    const std::string dylib_path = staging + "/wavetable_osc.dylib";
    ...
    if (!loader.load(dylib_path.c_str())) {
        // WavetableOsc links against WebGPU for thumbnail rendering; running
        // this test outside of an app context that links WebGPU produces
        // unresolved symbols. Skip gracefully in that case — the test still
        // gates correctness inside test_audio_correctness when run with
        // WebGPU available.
        std::fprintf(stderr, "  SKIP: could not load wavetable_osc.dylib (WebGPU unavailable)\n");
        return 0;
    }

    const auto* desc = loader.descriptor();
    check(desc != nullptr, "WavetableOsc descriptor not null");
    if (!desc) return 1;
    ...
    // Test 1: notes_in note-on produces audio in channel 0/1, channels 2..15 silent.
    {
        ...
        for (int ch = 2; ch < kMaxVoices && higher_zero; ++ch) {
    ...
    {
        std::fprintf(stderr, "\n--- WavetableOsc: note-off releases envelope ---\n");
        PolyTestContext tc;
        tc.set_output_channels(kMaxVoices);
        tc.clear_lane_ports();
        tc.clear_audio_inputs();
